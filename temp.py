            ...
            for form in branch:
                if isinstance(form, formula.Formula):
                    if form.is_atom and form.formula_one == '#':
                        ...
                        if isinstance(new_form, disjunction.Disjunction) or isinstance(new_form,
                                                                                       implication.Implication) or \
                                isinstance(new_form, box.Box) or isinstance(new_form, diamond.Diamond) or isinstance(
                            new_form, negation.Negation):
                            rank1.append(form)
                        elif isinstance(new_form, conjunction.Conjunction) or isinstance(new_form,
        ...
        while branch and not self.open_branch:
            ...
            if isinstance(branch[0], list) or (branch_one_opened and branch_two_opened):
                ...
                # a contradiction immediately closes the present branch
                    ...
                    return
                # apply branch rule
                elif not branch[0].is_atom and not (
                        isinstance(branch[0], negation.Negation) and branch[0].formula_one.is_atom):
                    ...
                    # for all other connectives, apply the rule, add it to the applied_rules list and remove from the active branch
                        ...
                        branch = branch[0].branch(branch, self)
                        new_rules = branch[0].branch(self)
